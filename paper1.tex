...
The value of $a_i$ for the first segment is set to be the value which is currently reported from sensor data.
...
The projection which is closest to the vehicle's actual position is then assumed to be the correct path projection $\bf{r}_p$.
...
So $s_{veh}$ at any epoch is then sum of the link lengths behind it's perpendicular projection$\bf{r}_p$.
...
In this case, an $s(t)$ spline has already been created using the control points from the unification process, and it is now resampled at $\Delta t = 0.01 sec$.
