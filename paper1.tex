...
To find closed-form solutions for each, a symbolic solver was used. The target values for each profile are the time steps between jerk changes.
These are solved by plugging in initial conditions, default jerk values
...
This results in negative time intervals for that phase.
...
Algorithm \ref{alg:modifyacceljerkpeaks} describes this procedure.
When it fails, a new jerk profile must be chosen.

\begin{figure}
  \begin{algorithmic}[1]
    \Procedure{ModifyAccelJerkPeaks}{$\Delta t$, $\mathbf{a}_m$, $\mathbf{j}_m$, $\mathbf{a}^{max}_m$, $\mathbf{j}^{max}_m$}
      
    \EndProcedure
  \end{algorithmic}
\caption{Procedure for modifying peak accel and jerk magnitude for rememdy negative time intervals for a given phase. This procedure is employed for each of the profiles.}
\label{alg:modifyacceljerkpeaks}
\end{figure}
