...
In this paper, we address the problem of trajectory planning for urban roads that satisfies 
constraints for human comfort while being reactive to the presence of pedestrians.
Past work on reducing jerk and limiting velocities and acceleration does not consider 
reactive behaviors such as responding to pedestrians. On the other hand, systems for collision 
avoidance, find and plan paths around obstacles and pedestrians, stop for them, or produce a warning. 
...
pedestrians on the road by slowing or stopping when no alternative paths are available. 
Our method has closed form solutions, and runs at 20 Hz ???????, and is efficient and reliable 
for use in online planning while optimizing rider comfort. We have confirmed this with
...
Computation of a tentative reference trajectory plan for traveling from one point to another is often guided by
passenger comfort. While the set of possible paths between any two points is infinite, it is 
typically heavily restricted by consideration of constraints such as traversability and legally available travel lanes, among others.
...
Each segment is then planned sequentially by choosing a piece-wise constant jerk profile.
...
In Section III, we describe trajectory planning which takes a 2D path and generates a trajectory that satisfies jerk and maximum velocity and acceleration. 
We explain how we handle intersections, turns in the road and planned stops due 
...
Integrating piece-wise constant jerk over time yields acceleration which is piece-wise linear over time, speed which is piece-wise quadratic over time, and position which is piece-wise cubic over time.
...
Since $j(t)$ is piece-wise constant, it may be integrated to determine an instantaneous trajectory at any moment in time.
...
Acceleration $a_{B,x}(t)$ is piece-wise linear, so lookup is a trivial matter.
...
Since it is known that $s(t)$ is piece-wise cubic, $t(s)$ may be approximated as cubic if a spline is being constructed from knots which have a very high resolution.
