...
exhibit reactive behavior and stop for pedestrians that appear on the road.
...
The task of Trajectory Planning is to take a 2 dimensional (horizontal plane) set of waypoints from the Path Manager, and compute a series of constant jerk intervals for motion along the length of the sub-path.
...
A path is then a set of $N_s$ segments, each of which is defined by a set of parameters $\mathbf{b}_k$, $\mathbf{a}_{m,k}$, and $\mathbf{j}_{m,k}$ where $k = 1 ... N_s$ and can be solved individually in sequence.
...
For each sub-path point, the value $v_{m,k}(a_{B,y}^{max})$ denotes the speed necessary to obtain the limit lateral acceleration, denoted by $a^{max}_{B,y}$.
