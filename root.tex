...
Other systems for collision avoidance plan paths around obstacles and pedestrians, stop for them, or produce a warning. 
...
When planning trajectories for an autonomous ground vehicle operating on roadways, the typical goal is to minimize navigation time.
...
Many systems in the literature find and plan paths around pedestrians, stop for them, or produce a warning ~\cite{pradalier05,benenson06,gu14,mogelmose15,johnson13} but do not work as part of an integrated system that can navigate urban roads.
These approaches also make the assumption that alternative paths around obstacles are available, which may be invalid in urban scenarios.
...
Our method first segments the path using a set of kinematic or dynamic constraints which may be functions of the desired path.
...
All detected pedestrian locations are compared against the high-resolution digital map, so that pedestrians that are off roadway are not considered.
Then each pedestrian location is projected onto the 1D path in the same manner as described in Sec. \ref{sec:trajectorytracking}.
...
The procedure for doing this is the same as the procedure for planning the RSTOP trajectory with fewer steps, since the planning algorithm is extremely efficient.
...
\end{enumerate}
After these steps are completed, the process for fitting jerk profiles described in next section may be applied to each of the segments so that a plan of jerk vs. time may be generated.
...
$v_{m,k_p}(a_{B,y}) = \sqrt{a_{B,y}^{max}/\kappa_{k_p}}$ for $k_p = 1, ..., N_p$, where $N_p$ is the number of waypoints in the sub-path.
...
Any number of clustering algorithms may be used here, and a prudent choice of segment boundaries is arguably one of the most critical pieces of trajectory planning process.
We choose a simple univariate heuristic which essentially identifies curves in the roadway using only the speed ceiling.
...
A jerk vs. time profile is fit to each segment using the algorithm described in the next section.
...
the urban path. We then explained how our trajectory plan is regularly supplied in real-time to the
