...
Past work limits jerk, velocities and acceleration for smooth trajectories, but does not consider reactive behaviors such as responding to pedestrians.
Other systems for collision avoidance stop or plan paths around obstacles and pedestrians.
...
We present a novel online method for planning longitudinal trajectories to follow an urban path with turns and intersections while honoring traffic regulations such as stop signs. 
Our method updates the trajectory to safely avoid pedestrians on the road by slowing down or stopping.
This is critical in urban environments, where modifying the vehicle's intended path is often not possible.
...
When planning trajectories for an autonomous ground vehicle operating on roadways, the typical goal is minimization of navigation time.
% "path-trajectory" is improper. "path-velocity" is used extensively in the literature.
It is common to employ the path-trajectory decomposition to separate the two tasks so that they can be carried out sequentially or in parallel.
...
The second task is {\it Trajectory Planning}, which involves the computation of a velocity profile appropriate for the scenario.
...
Trajectory computation for traveling from one point to another is often guided by passenger comfort. 
...
Similar work has been performed by Bianco et al \cite{GuarinoLoBianco2004,GuarinoLoBianco2005,Bianco2007,Bianco2009,GuarinoLoBianco2013}, which sets the travel time a priori and optimally minimizes without hard limitations or direct jerk control.
Other work on optimizing these constraints on urban roads ~\cite{Rastelli14,Li15} doesn't consider reactive behaviors such as responding to pedestrians or other dynamic objects in the scene.

Many systems in the literature find and plan paths around pedestrians, stop for them, or produce a warning ~\cite{pradalier05,benenson06,gu14,mogelmose15,johnson13} but do not work as part of an integrated system that navigates urban roads.
These approaches also make the assumption that alternative paths around obstacles are available, which may not always be valid.
Planners that optimize human comfort by modifying the path seek obstacle-free space \cite{Villagra2012,Villagra2012a} and do not feature stopping as a well-integrated behavior.
In this paper, we describe a trajectory generation system that satisfies the constraints for human comfort while being reactive to presence of pedestrians on the road. We enforce hard upper limits on jerk without requiring that travel time be known before planning occurs.
...
At stop sign, the autonomous car should stop and wait for an appropriate amount of time.
...
We present a novel method for planning longitudinal trajectories (i.e., the time derivatives of position in the body-forward direction) along the desired path without requiring prior knowledge of travel time.
...
The jerk profile is intelligently chosen from a set of pre-solved profiles such that acceleration is continuous and within limits throughout the entire path.
...
We present an online method for revising the trajectory plan to safely avoid pedestrians that enter the roadway by slowing down or stopping when no alternative paths are available.
...
We explain how we pass through intersections, turns in the road and planned stops due to stop signs.
...
Beyond these major components, our system has 4 primary input components: Navigation Filter, Pedestrian Detector, a Path Planner, and a Map which provides the location of stop signs.
% nav filter
...
We assume access to a path planner.
...
Our map is used to search for location of stop signs.
...
Path Manager performs trajectory planning over a sliding window within the full path which progresses with the vehicle.
It breaks the path into sub-paths at each stop location which is known a priori, as shown in Fig. \ref{fig:subpathdivision}, and call the Trajectory Planner for each sub-path in the path.
...
Each of the states are described below.
...
We first break each sub-path into segments with simple shape.
...
\begin{figure}[thpb]
  \centering
  \subfigure[
    Result of dividing a single sub-path into separate segments.
  ...
  \\
  \subfigure[
    Example of how the sub-path's segment boundaries are determined, as well as how $v_m$ is consolidated to determine the speed ceiling for each segment.
...
To summarize, in the last three sections we described how a urban path is broken into sub-paths. We 
