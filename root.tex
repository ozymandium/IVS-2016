...
In this paper, we describe a trajectory generation system that satisfies these constraints for human comfort and is reactive to presence of pedestrians on the road.
...
The task of Trajectory Planning is to take a 2 dimensional (horizontal plane) set of waypoints from the Path Manager, and compute a series of constant jerk intervals for motion along the length of the path.
...
%$v_m$, $\mathbf{a}_m$, and $\mathbf{j}_m$ 
...
Lateral acceleration constraints are translated into longitudinal speed constraints using the approximation $v_{m,k}(a_{B,y}) = \sqrt{a_{B,y}^{max}/\kappa_k}$ for $k = 1, ..., N_p$, where $N_p$ is the number of waypoints in the subpath.
The value $\kappa_k$ denotes the path curvature at each point.
For each \textbf{??? sub-path ??} point, the value $v_{m,k}(a_{B,y}^{max})$ denotes the speed necessary to obtain the limit lateral acceleration, denoted by $a^{max}_{B,y}$.
...
%$\mathbf{g}$, $\mathbf{a}_m$, and $\mathbf{j}_m$ 
initial/final speeds, speed limit, peak acceleration and jerk 
...
If $\mathbf{a}_m = [a^+_m , a^-_m]$ and $\mathbf{j}_m = [j^+_m , j^-_m]$. are known, then a profile with $M$ jerk phases (7 in this case) can be generated by solving for the time intervals $[\Delta t_1, ..., \Delta t_M]$.
Jerk and peak acceleration may be different when reducing speed versus increasing speed to more closely mirror human driving behavior.
...
This is computed for every point along the subpath.
...
\textbf{RG: does not belong here: The other profiles are explained below in Section \ref{sec:jerkprofiles}.
A path is then a set of $N_s$ segments, each of which is defined by a set of parameters $\mathbf{b}_k, k = 1 ... N_s$ and can be solved individually in sequence. Determination of where to break a path into segments is discussed below in Section \ref{sec:pathsegmentation}. }
...
To find closed-form solutions for each, a symbolic solver is used.
...
\subsection{Stitching segment profiles together} \label{sec:unifiedsolution}

At this point, each sub-path has been divided into segments and conditions have been computed such that an analytically derived plan can be fit to each segment individually.
Now a reference trajectory must be generated for the entire sub-path.
...
\textbf{RG: please explain reference time, $s$ and $\Delta s$ ????????}
...
Since it is known that $s(t)$ is piecewise cubic, reference time as a function of vehicle position along the path, $t(s)$, F may be approximated \textbf {?????? as cubic if a spline is ???? } being constructed from knots which have a very high resolution.
